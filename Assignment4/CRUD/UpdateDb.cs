...
namespace Assignment4.CRUD
{
    public class UpdateDb
    {
        ...
        public void UpdateOperation(object obj, string foreign_key, string foreignKeyRefID)
        {
            ...
            var foreignKey = properties.FirstOrDefault(x => x.Name.ToLower() == "id").GetValue(obj);


            string? foreignKeyID = foreignKey.ToString();

            if (foreign_key != null && foreignKeyRefID != null)
            {
                dict.Add(foreign_key, foreignKeyRefID);
            }
            ...
            foreach (PropertyInfo property in properties)
            {
                ...
                else if (value is IList list)
                {
                    ...
                    {
                        UpdateOperation(item, foreignKeyName, foreignKeyID);
                    }
                }
                else
                {
                    ...
                    // ...
                    UpdateOperation(value, foreignKeyName, foreignKeyID);
                }
            }

            // Generate and execute the update SQL statement
            // ...
            GenerateUpdateSql(tableName, dict, foreignKeyID);
        }
